...
# ==============================================================================
# ### 2단계: 커스텀 "확장" 모델 및 토크나이저 로딩 (✅ 핵심 수정 지점)
# ==============================================================================
print("\n===== [2단계] Script 1에서 저장한 확장 모델/토크나이저 로딩 =====")

# ✅ 1. Script 1에서 저장한 *확장된* 모델의 경로를 지정합니다.
extended_model_path = "./KoBERT-Extended-with-Dialect" 

# ✅ 2. 해당 경로에서 확장된 토크나이저와 모델을 바로 로드합니다.
#    (Script 1에서 이미 어휘 추가와 리사이즈가 완료된 상태입니다.)
...
# 🚫 이전 코드의 new_tokens, add_tokens, resize_token_embeddings 등
#    불필요하고 잘못된 중복 로직을 모두 제거했습니다.
...
training_args = TrainingArguments(
    ...
    learning_rate=5e-5,
    warmup_steps=300, # 3000 -> 300 (데이터가 적으므로)
    weight_decay=0.01,
    logging_dir="./logs_custom_tokenizer",
    logging_steps=100, # 10000 -> 100
    evaluation_strategy="steps",
    eval_steps=100, # 10000 -> 100
    save_total_limit=2,
    save_steps=100, # 10000 -> 100
