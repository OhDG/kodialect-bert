...
with open("dialect_spm.vocab", "r", encoding="utf-8") as f:
    for line in f:
        ...
        # 2. ✅ " " (U+2581)로 시작하는 *유의미한* 토큰이면서
        #    (이전 코드 'not piece.startswith(" ")'는 잘못된 로직이었음)
        if piece.startswith(" "):
            # 3. ✅ KoBERT 원본에 없는 토큰만 추출
            if piece not in original_vocab:
...
# 검증 로직 (이전과 동일, UNK가 줄어드는지 확인)
test_sentences = [
    "점심은 잡솼슈? 어데 가노? 마씸 참말이우꽈?",
    "오늘 날씨 진짜 좋다 아이가?",
    "그거 뭐라카노? 내가 다 했다 아이가!",
    "니 집에 언제 올껴?",
    "거 참말로 오지게 좋구마잉!",
...
print("\n--- KoBERT 토크나이저 검증 ---")
# 원본 KoBERT를 다시 로드해서 비교
base_tokenizer = AutoTokenizer.from_pretrained(base_model_name, trust_remote_code=True)
for sentence in test_sentences:
    tokens_before = base_tokenizer.tokenize(sentence)
    tokens_after = original_tokenizer.tokenize(sentence) # original_tokenizer는 확장된 버전
