    ...
    if spec["type"] == "local_wordpiece":
        ...
        if not vocab_path.exists():
            raise FileNotFoundError(
                f"vocab.txt를 찾을 수 없습니다: {vocab_path}\n"
                f"먼저 step2_train_tokenizer_final.py를 실행해 tokenizer를 학습하세요."
            )
...
def compute_zipf_metrics(token_counter, rank_log_cutoff=6.0):
    ...
    Cardinality: tokenization 이후 실제 등장한 unique token 수
    Zipf AUC: log(rank)-log(frequency) 곡선 아래 면적
    Zipf Slope: log(rank)-log(frequency) 선형 근사 기울기
    Power-law Error: 실제 log-frequency와 선형 근사값 간 평균절대오차

    논문 기준: AUC, Slope, Power-law Error는 log(rank) <= 6 구간에서 계산
    """
...
def compare_tokenizers(corpus_path, batch_size=1024, max_lines=None, rank_log_cutoff=6.0, output_dir="./tokenizer_eval_results"):
    ...
    if not corpus_path.exists():
        raise FileNotFoundError(
            f"평가 corpus 파일을 찾을 수 없습니다: {corpus_path}\n"
            f"먼저 step1_prepare_data_final.py를 실행해 dialect_eval_corpus.txt를 생성하세요."
        )
    ...
    for tokenizer_name, spec in TOKENIZER_SPECS.items():
        ...
        zipf_metrics = compute_zipf_metrics(stats["token_counter"], rank_log_cutoff=rank_log_cutoff)

        row = {**basic_metrics, **zipf_metrics}
        ...
        save_zipf_curve(tokenizer_name, stats["token_counter"], output_dir)
...
def parse_args():
    parser = argparse.ArgumentParser(description="held-out eval corpus에서 3개 tokenizer를 비교 평가합니다.")
    parser.add_argument("--corpus", type=str, default="dialect_eval_corpus.txt", help="평가용 사투리 corpus txt 파일")
    parser.add_argument("--batch_size", type=int, default=1024)
    parser.add_argument("--max_lines", type=int, default=None, help="빠른 테스트용. 전체 평가 시 생략")
    parser.add_argument("--rank_log_cutoff", type=float, default=6.0, help="[14] 논문 기준 log(rank) cutoff")
    parser.add_argument("--output_dir", type=str, default="./tokenizer_eval_results")
