...
DEFAULT_DIALECT_TOKENIZER_DIR = "../shared/dialect_bert_tokenizer"
DEFAULT_KLUE_TOKENIZER_NAME = "klue/bert-base"
DEFAULT_TRAIN_CORPUS = "../shared/corpus/dialect_train_corpus.txt"
DEFAULT_EVAL_CORPUS = "../shared/corpus/dialect_eval_corpus.txt"
DEFAULT_OUTPUT_DIR = "./small_bert_mlm_continued"
...
def load_dialect_tokenizer(tokenizer_dir: str, max_length: int) -> BertTokenizerFast:
    ...
    if not vocab_path.exists():
        raise FileNotFoundError(
            ...
            "Place vocab.txt at ../shared/dialect_bert_tokenizer/vocab.txt or pass --dialect_tokenizer_dir."
        )
...
def load_mlm_model_and_tokenizer(args: argparse.Namespace):
    init_model_dir = None
    if args.init_model_dir:
        init_model_dir = resolve_existing_path(args.init_model_dir, required=True)
        tokenizer = AutoTokenizer.from_pretrained(str(init_model_dir), use_fast=True)
        tokenizer.model_max_length = args.max_length
        model = BertForMaskedLM.from_pretrained(str(init_model_dir))
        config = model.config

        if config.vocab_size != len(tokenizer):
            raise ValueError(
                f"Tokenizer/model vocab mismatch: tokenizer={len(tokenizer)}, model={config.vocab_size}"
            )
        if tokenizer.pad_token_id is not None:
            model.config.pad_token_id = tokenizer.pad_token_id
        return tokenizer, model, config, str(init_model_dir), str(init_model_dir)

    tokenizer, tokenizer_source = load_experiment_tokenizer(args)
    config = build_small_bert_config(args, tokenizer)
    model = BertForMaskedLM(config)
    return tokenizer, model, config, tokenizer_source, None
...
def load_and_tokenize_corpus(args: argparse.Namespace, tokenizer):
    ...
    train_corpus = resolve_existing_path(
        args.train_corpus,
        "../shared/corpus/dialect_train_corpus.txt",
        "./dialect_train_corpus.txt",
        "../260630_test_1/dialect_train_corpus.txt",
    )
    eval_corpus = resolve_existing_path(
        args.eval_corpus,
        "../shared/corpus/dialect_eval_corpus.txt",
        "./dialect_eval_corpus.txt",
...
def run_mlm_pretraining(args: argparse.Namespace) -> None:
    ...
    tokenizer, model, config, tokenizer_source, init_model_dir = load_mlm_model_and_tokenizer(args)
    ...
    run_type = "continued_pretraining" if init_model_dir else "from_scratch"
    print("\n--- Small BERT MLM pretraining start ---")
    print(f"run_type: {run_type}")
    print(f"tokenizer_mode: {args.tokenizer_mode}")
    print(f"tokenizer_source: {tokenizer_source}")
    print(f"init_model_dir: {init_model_dir}")
    ...
    metadata = {
        "training_type": f"small_bert_mlm_{run_type}",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Pretrain or continue-pretrain a small BERT MLM model with either dialect or KLUE-BERT tokenizer."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional MLM final_model directory to continue from. If omitted, initializes from scratch.",
    )
