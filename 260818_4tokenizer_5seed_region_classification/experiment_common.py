...
def reset_process_gpu_memory_stats() -> None:
    """Reset CUDA peaks tracked by this Python process only."""
    if not torch.cuda.is_available():
        return
...
def collect_process_gpu_memory_stats() -> Dict[str, object]:
    """Return allocator metrics that cannot include another process."""
    if not torch.cuda.is_available():
        return {
            "available": False,
            "measurement_scope": "current_python_process_only",
        }
    ...
    return {
        ...
        "peak_allocated_mib": torch.cuda.max_memory_allocated(device_index) / mib,
        "peak_reserved_mib": torch.cuda.max_memory_reserved(device_index) / mib,
        "current_allocated_mib": torch.cuda.memory_allocated(device_index) / mib,
        "current_reserved_mib": torch.cuda.memory_reserved(device_index) / mib,
    }
