...
def optional_mean_std(values: List[object]):
    numeric = [float(value) for value in values if value is not None]
    return mean_std(numeric) if numeric else None
...
def collect_results(args: argparse.Namespace) -> Dict[str, object]:
    ...
    for tokenizer_name in args.tokenizers:
            ...
            seed_results.append(
                {
                    ...
                    "gpu": gpu,
                    "process_gpu_memory": metadata.get("process_gpu_memory", {}),
                }
            )
        ...
        results["tokenizers"][tokenizer_name] = {
            ...
            "mlm_gpu": mlm_gpu,
            "mlm_process_gpu_memory": mlm_metadata.get("process_gpu_memory", {}),
            "runs": seed_results,
            "summary": summary,
            ...
            "efficiency": {
                ...
                "classification_process_sm_utilization_percent": optional_mean_std(
                    [item["gpu"].get("average_active_process_sm_utilization_percent") for item in seed_results]
                ),
                "classification_peak_process_allocated_mib": max(
                    float(item["process_gpu_memory"].get("peak_allocated_mib", 0.0))
                    for item in seed_results
                ),
                "classification_peak_process_reserved_mib": max(
                    float(item["process_gpu_memory"].get("peak_reserved_mib", 0.0))
                    for item in seed_results
                ),
            },
        }
    ...
    with (output_dir / "efficiency_summary.csv").open("w", encoding="utf-8-sig", newline="") as f:
        ...
        writer.writerow(
            [
                ...
                "mlm_process_sm_utilization_percent",
                "mlm_peak_process_allocated_mib",
                "mlm_peak_process_reserved_mib",
                "classification_train_runtime_mean_seconds",
                "classification_train_runtime_std_seconds",
                "classification_process_sm_utilization_percent",
                "classification_peak_process_allocated_mib",
                "classification_peak_process_reserved_mib",
                "test_runtime_mean_seconds",
            ]
        )
        for tokenizer_name, tokenizer_result in results["tokenizers"].items():
            efficiency = tokenizer_result["efficiency"]
            mlm_process_memory = tokenizer_result["mlm_process_gpu_memory"]
            mlm_process_util = tokenizer_result["mlm_gpu"].get(
                "average_active_process_sm_utilization_percent"
            )
            classifier_process_util = efficiency[
                "classification_process_sm_utilization_percent"
            ]
            writer.writerow(
                [
                    ...
                    mlm_process_util,
                    mlm_process_memory.get("peak_allocated_mib"),
                    mlm_process_memory.get("peak_reserved_mib"),
                    efficiency["classification_train_runtime"]["mean"],
                    efficiency["classification_train_runtime"]["std"],
                    classifier_process_util["mean"] if classifier_process_util else None,
                    efficiency["classification_peak_process_allocated_mib"],
                    efficiency["classification_peak_process_reserved_mib"],
...
def write_markdown(results: Dict[str, object], output_path: Path) -> None:
    ...
    lines.extend(
        [
            ...
            "",
            "All VRAM values below are measured for the experiment Python process only. "
            "Process SM utilization is shown as N/A when Windows WDDM does not expose it through NVML.",
            "",
            "| Tokenizer | Vocab | Params | MLM time | MLM process SM | MLM process peak VRAM | Classifier time | Classifier process SM | Classifier process peak VRAM | Test time |",
            "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
        ]
    )
    for tokenizer_name, tokenizer_result in results["tokenizers"].items():
        ...
        lines.append(
            ...
            f"{format_optional_percent(mlm_gpu.get('average_active_process_sm_utilization_percent'))} | "
            f"{float(mlm_process_memory.get('peak_reserved_mib', 0.0)) / 1024:.2f} GB | "
            f"{efficiency['classification_train_runtime']['mean'] / 3600:.2f} +/- "
            f"{efficiency['classification_train_runtime']['std'] / 3600:.2f} h | "
            f"{format_optional_percent(classifier_process_util['mean'] if classifier_process_util else None)} | "
            f"{efficiency['classification_peak_process_reserved_mib'] / 1024:.2f} GB | "
