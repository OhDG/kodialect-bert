...
# mBERT's 119,547-way vocabulary makes the MLM head's [batch, seq_len, vocab_size]
# logits tensor far bigger than the other three tokenizers' (32,000 or 8,002-way).
# Its MLM micro batch is halved and gradient accumulation doubled so the
# effective batch (256) stays identical across all four tokenizers; only the
# per-step memory footprint differs, which is an intrinsic property of the
# vocabulary being compared, not an experimental confound.
LARGE_VOCAB_TOKENIZERS = {"mbert"}
...
def summarize_gpu_csv(csv_path: Path, wall_seconds: float) -> Dict[str, object]:
    ...
    return {
        ...
        "note": (
            ...
            "queries typically report real per-process VRAM/SM usage (unlike Windows "
            "WDDM, which often withholds them)."
        ),
    }
...
def preflight(args: argparse.Namespace) -> None:
    ...
    if missing:
        raise RuntimeError(
            ...
            "Run: python -m pip install -r requirements_260818.txt"
        )
    ...
    if memory_gb < 40 and not args.smoke:
        raise RuntimeError(
            "This profile is tuned for an RTX A6000-class 48 GB GPU. "
            "Use --smoke for a compatibility check, or lower --*_batch_size "
            "overrides for a smaller card."
        )
    data_dir = ROOT / data_root(args)
    if prepared_data_complete(data_dir):
        print(f"[PREFLIGHT] Reusing prepared 80/10/10 data: {data_dir}")
    elif not Path(args.source_manifest).is_file() and not (ROOT / args.source_manifest).is_file():
        raise FileNotFoundError(f"Source manifest not found: {args.source_manifest}")


def prepared_data_complete(data_dir: Path) -> bool:
    required = [
        data_dir / "corpus" / "dialect_train_corpus.txt",
        data_dir / "corpus" / "dialect_validation_corpus.txt",
        data_dir / "corpus" / "dialect_test_corpus.txt",
        data_dir / "region_classification" / "dialect_region_train.tsv",
        data_dir / "region_classification" / "dialect_region_validation.tsv",
        data_dir / "region_classification" / "dialect_region_test.tsv",
    ]
...
def classifier_command(
    ...
    command = [
        ...
        "--train_batch_size",
        "16" if args.smoke else str(args.classifier_train_batch_size),
        "--eval_batch_size",
        "32" if args.smoke else str(args.classifier_eval_batch_size),
        "--gradient_accumulation_steps",
...
def main() -> None:
    ...
    prepared_data_dir = ROOT / data_root(args)
    if prepared_data_complete(prepared_data_dir) and not args.overwrite:
        print(f"[SKIP] Reusing prepared 80/10/10 data: {prepared_data_dir}")
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the RTX A6000 48 GB four-tokenizer, five-seed region "
            "classification experiment with one command."
        )
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=256,
        help="Micro batch for classifier fine-tuning (same for all four tokenizers: the classifier head is 5-way, not vocab-sized).",
    )
    parser.add_argument("--classifier_eval_batch_size", type=int, default=4096)
